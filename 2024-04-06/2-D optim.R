...
h <- function(x) {
    ...
    return(exp(-(x - 0.5)^2 / (2 * 0.25))) # norm(0.5, 0.5)
...
start <- Sys.time()
for (k in 1:step) {
    # all_res <- mclapply(1:nrow(Xstart), local_opt_fun, Xstart = Xstart, XN = XN, mc.cores = 1)
...
open3d()
plot3d(x1, x2, z, zlab = "real function")
open3d()
plot3d(x1, x2, pm1, zlab = "Laplace")
